...
Object.defineProperty(ViewBase.prototype, 'cssClasses', {
  ...
  get(this: ViewBase) {
    ...
    if (!cssClasses.has(platformClass)) {
      cssClasses.add(platformClass);
    }

    if (!cssClasses.has(activeFontScaleClass.cssClass) || !cssClasses.has(activeFontScaleClass.shortHandCssClass)) {
      for (const { cssClass, shortHandCssClass } of fontScaleCssClasses.values()) {
        cssClasses.delete(cssClass);
        cssClasses.delete(shortHandCssClass);
      }

      cssClasses.add(activeFontScaleClass.cssClass);
      cssClasses.add(activeFontScaleClass.shortHandCssClass);
    }
