...
import { Application, CSSUtils, isAndroid, PropertyChangeData, View } from '@nativescript/core';
...
class A11YCssClassHelper {
  ...
  constructor() {
    ...
    hmrSafeEvents(`${this.cls}.updateRootViews`, [Application.displayedEvent, Application.resumeEvent], Application, (evt) => {
  ...
  private removeCssClass(cssClass: string) {
    ...
    [Application.getRootView(), ...this.getModalViews()].forEach((view) => {
      if (view) {
